    ...
    with feed_col1:
        ...
        if display_alerts:
            for alert in display_alerts[:5]:
                ...
                safe_timestamp = html.escape(str(alert.get('timestamp', '')))
                ...
                <div class="threat-item {sev_cls}">
                    ...
                    <div style="font-size:0.78rem; color:#8B949E; font-family:'JetBrains Mono', monospace;">
                        Src IP: <span style="color:#58A6FF;">{safe_src_ip}</span> | Account: <span style="color:#E6EDF3;">{safe_username}</span> | Target: {safe_affected_asset}
                    </div>
                    <div style="font-size:0.75rem; color:#6E7681; margin-top:2px;">
                        Timestamp: {safe_timestamp} | MITRE: <b>{safe_mitre_id}</b> ({safe_mitre_name}) | Confidence: {safe_confidence}%
                    </div>
                </div>
                """
