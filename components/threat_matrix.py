...
def render_threat_matrix(alerts: list, ai_assistant, logs_df: pd.DataFrame):
    ...
    with col1:
        ...
        for idx, alert in enumerate(alerts):
            ...
            safe_confidence = html.escape(str(alert.get('confidence', '')))
            safe_src_ip = html.escape(str(alert.get('src_ip', '')))
            safe_username = html.escape(str(alert.get('username', '')))
            safe_affected_asset = html.escape(str(alert.get('affected_asset', '')))
            ...
            safe_description = html.escape(str(alert.get('description', '')))

            with st.expander(f"[{safe_alert_id}] {safe_title} — ({sev.upper()})", expanded=(idx == 0)):
                st.markdown(f"""
                    <div style="font-size: 0.82rem; margin-bottom: 8px;">
                        <b>Severity:</b> {badge_html} | <b>Confidence:</b> <span style="color:#58A6FF; font-family:'JetBrains Mono';">{safe_confidence}%</span>
                    </div>
                    <div style="font-size: 0.8rem; color: #8B949E; font-family: 'JetBrains Mono', monospace; margin-bottom: 8px;">
                        Source IP: <span style="color:#58A6FF;">{safe_src_ip}</span> | User: <span>{safe_username}</span> | Asset: {safe_affected_asset}
                    </div>
    ...
    with col2:
        ...
        if selected:
            ...
            safe_sel_alert_id = html.escape(str(selected.get('alert_id', '')))
            safe_sel_confidence = html.escape(str(selected.get('confidence', '')))
            safe_sel_src_ip = html.escape(str(selected.get('src_ip', '')))
            safe_sel_username = html.escape(str(selected.get('username', '')))
            safe_sel_affected_asset = html.escape(str(selected.get('affected_asset', '')))
            safe_sel_mitre_id = html.escape(str(selected.get('mitre_id', '')))
            safe_sel_mitre_name = html.escape(str(selected.get('mitre_name', '')))
            safe_sel_recommendation = html.escape(str(selected.get('recommendation', '')))
                ...
                <div class="soc-panel">
                    ...
                    <div style="margin-bottom: 12px;">
                        ...
                        <span style="font-size: 0.75rem; color: #8B949E; margin-left: 8px; font-family:'JetBrains Mono';">
                            ID: {safe_sel_alert_id} | Confidence: {safe_sel_confidence}%
                        </span>
                    </div>
                    ...
                    <div style="font-size: 0.8rem; color: #8B949E; margin-bottom: 4px;">
                        <b>Destination Asset:</b> {safe_sel_affected_asset}
                    </div>
